...
`agent` user, ephemeral `--rm` container, repo clone only). Every harness
...
`2026.08.25-3e8eec8` (curl installer, unpinned).
...
Git: clone/pull uses a host-scoped `http.extraheader` (never write the PAT
into the remote URL / `.git/config`). The same env is installed on the
adapter process so harness `git fetch/pull/push` can use it.
`GIT_TERMINAL_PROMPT=0`. HTTPS push to that host works only if the PAT has
write scope. SSH remotes have no extraheader and there is no ssh-agent in
the box.
